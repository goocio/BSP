...
- `\eqref[]{}`: is to `amsmath`'s `\eqref` what `\pref` is to `\ref`, in that it makes the entire thing hyperlinked; additionally you can pass an optional argument, in case e.g. if equation (2.6) is labelled with `\label{eq:system of pdes}` and rather than "(2.6)" being hyperlinked you want "system (2.6)" hyperlinked, you can do `\eqref[system]{eq:system of pdes}`;
